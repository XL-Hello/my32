...
本组件使用项目内 `components/third_party/led_strip` 驱动 WS2812B 灯珠，对外接口不依赖后端。默认使用 RMT；将 CMake 配置 `WS2812B_BACKEND` 设为 `SPI` 后，改用 `led_strip` SPI 后端。`led_strip` v2.5.5 源码已随项目保存，构建不依赖 Component Manager 或网络注册表。
