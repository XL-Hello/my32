...
默认接线适用于 YD-ESP32-S3-COREBOARD，使用当前 LCD、触摸和 RGB LED 配置之外的 GPIO。

| AHT20 引脚 | ESP32-S3 引脚 | 说明 |
| --- | --- | --- |
| VCC | 3V3（J1-1 或 J1-2） | 仅使用 3.3 V 供电。 |
| GND | GND（J1-22 或 J2-1） | 与开发板共地。 |
| SDA | GPIO2（J2-5） | I2C 数据线。 |
| SCL | GPIO1（J2-4） | I2C 时钟线。 |
...
AHT20_I2C_SDA_GPIO  # 默认 GPIO2
AHT20_I2C_SCL_GPIO  # 默认 GPIO1
```
...
```text
I (...) aht20: (aht20_init:...): AHT20 initialized: I2C0 SDA=GPIO2 SCL=GPIO1 address=0x38
